...
public int recursiveSearch(E target) 
{
    return recursiveSearch(head, target, 0);
}

private int recursiveSearch(Node<E> node, E target, int index)
{
    if (node == null) 
    {
        // Base case: end of the list, element not found
        return -1; 
    }
    ...
    // Recursively search the next node with incremented index
    return recursiveSearch(node.next, target, index + 1);
}
